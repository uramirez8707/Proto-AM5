...
use atmos_ozone_tracer_mod,only : atmos_ozone_tracer_init, atmos_ozone_tracer, &
                                  atmos_ozone_tracer_endts, &
                                  atmos_ozone_tracer_time_vary, &
...
!-----------------------------------------------------------------------
!----------- namelist -------------------
logical :: prevent_flux_through_ice = .true.  , step_update_tracer = .false.
...
logical  :: do_ozone_tracer = .false. ! If set to .true., ozone is calculated from the linear scheme.
namelist /atmos_tracer_driver_nml / prevent_flux_through_ice, step_update_tracer, do_esm_nitrogen_flux,do_nh3_atm_ocean_exchange, &
        do_cmip6_bug_diag, do_ozone_tracer
...
!   ozone tracer
!------------------------------------------------------------------------
   if ( do_ozone_tracer ) then
      call mpp_clock_begin ( ozone_tracer_clock )
      if ( no3 > 0 ) then
         if ( no3 > nt ) call error_mesg ('Tracer_driver', &
            'Number of tracers .lt. number for ozone tracer', FATAL)
         call atmos_ozone_tracer ( lon, lat, phalf, &
               tracer(:,:,:,no3), rtnd, Time, dt, is, js, kbot )
         rdt(:,:,:,no3) = rdt(:,:,:,no3)+rtnd(:,:,:)
         if ( step_update_tracer ) then
            tracer(:,:,:,no3) = tracer(:,:,:,no3) + rtnd(:,:,:)*dt
         endif
      endif
      call mpp_clock_end (ozone_tracer_clock)
   endif
...
!>
      type(interpolate_type) :: o3ddep_data
...
!---------------------------------------------------------------------
! Initiate ozone tracer
!--------------------------------------------------------------------
     if ( do_ozone_tracer) then
             call atmos_ozone_tracer_init (domain, r, axes, Time, no3, lonb, latb, phalf, &
                                   mask, o3ddep_data)
        drydep_data(no3) = o3ddep_data
        ozone_tracer_clock = mpp_clock_id ('Tracer: Ozone tracer',&
                                            grain=CLOCK_MODULE )
     endif
...
type(time_type), intent(in) :: Time
      ...
      if (do_ozone_tracer) then
        call atmos_ozone_tracer_time_vary (Time)
      endif
