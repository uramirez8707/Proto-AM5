...
!   use_online_aerosol the online aerosol fields should be used for nucleation source
!                      (rather than climo values) ?
!   use_sub_seasalt  only sub-micron seasalt particles should be used for nucleation ?
!                    sea_salt_scale" UNITS="" TYPE="real" DEFAULT="0.1">
!                    scaling factor used when offline seasalt aerosol is used for
!                    nucleation
!   om_to_oc  scaling factor used to convert offline organic matter to organic
!             carbon for nucleation
!
logical :: do_unified_clouds = .false. 
logical :: do_lsc = .false.
logical :: do_uw_conv=.true.
real    :: pdepth = 150.e2
...
integer, dimension(:), allocatable :: id_wetdep_uw
...
subroutine moist_processes_time_vary (Time_in, dt_in, i_shallow)

real, intent(in) :: dt_in
type(time_type), intent(in) :: Time_in
integer, intent(in) :: i_shallow

!-----------------------------------------------------------------------

      dt = dt_in
      call convection_driver_time_vary (Time_in, dt_in, i_shallow)
...
!    the COSP simulator.
!------------------------------------------------------------------------
      if (trim(cosp_precip_sources)  == 'stratuw') then
...
!    fluxes.
!----------------------------------------------------------------------
      do k=1, size(Removal_mp%rain3d,3)-1
        do j=1, size(Removal_mp%rain3d,2)
          do i=1, size(Removal_mp%rain3d,1)
            ii = i+is-1
            jj = j+js-1
...
!---------------------------------------------------------------------
      if (id_rh_cmip > 0) then
        call rh_calc (Input_mp%pfull, input_mp%tin, Input_mp%qin, RH, do_cmip=.true.)
...
!------------------------------------------------------------------------
      ...
      Input_mp%qturbten => Phys_mp_exch%qturbten       ! ZNT: 06/08/2021
...
!------------------------------------------------------------------------
      ...
      Input_mp%qturbten => null()       ! ZNT: 06/08/2021
...
!   local variables:
      ...
      character(len=128)    :: diaglname, diaglname_uw
