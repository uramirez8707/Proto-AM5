...
logical :: rk_act_only_if_ql_gt_qmin = .true.   ! compute qn source term
                                                ! (activated nuclei)
                                                ! only when ql is > qmin ?
logical :: include_homogeneous_for_wetdep = .false. 
                                                ! treat homogeneous 
...
! rthresh      UNITS="microns"  liquid cloud drop radius threshold for autoconversion.
! use_kk_auto  the Khairoutdinov and Kogan (2000) autoconversion should be used?
! U_evap       critical relative humidity above which rain does not evaporate.
! U_evap_snow  critical relative humidity above which snow does not evaporate.
! tracer_advec cloud liquid, ice and fraction are advected by the grid resolved motion?
! vfact"       factor used to enhance ice fall velocity.
! iwc_crit UNITS="kg(ice)/m**3  critical ice-water content below which to apply alternate fall
!                               speed formula
! vfall_const2  factor for alternate fall speed formula
! vfall_exp2    exponent for alternate fall speed formula
! num_mass_ratio1
! num_mass_ratio2
! super_choice    excess vapor in supersaturated conditions should be put into
