...
!--> h1g-2020-02-20 save the CAM default values
...
!real(r8), parameter :: lam_bnd_snow(2) = 1._r8/[2000.e-6_r8, 10.e-6_r8]  ! h1g, 2020-07-30
...
subroutine micro_mg_utils_init( kind, rh2o, cpair, tmelt_in, latvap, &
  ...
  real(r8) :: lam_bnd_snow(2)       ! h1g, 2020-07-30
  !-----------------------------------------------------------------------
  ...
  lam_bnd_snow      = 1._r8/[2000.e-6_r8, 2._r8*dcs]  ! h1g, 2020-07-30
!  lam_bnd_snow      = 1._r8/[5000.e-6_r8, 2._r8*dcs]  ! h1g, 2020-08-08
...
function rising_factorial_r8(x, n) result(res)
...
function rising_factorial_integer(x, n) result(res)
  real(r8), intent(in) :: x
  integer, intent(in) :: n
  real(r8) :: res
     ...
     if (props_loc%eff_dim == 3._r8) then
        props_loc%shape_coef = pi / 6._r8 * props_loc%rho * &
         !    rising_factorial(pgam+1._r8, 3)
              gamma(pgam+4._r8)/gamma(pgam+1._r8)    ! h1g, 2016-12-09
     else
        props_loc%shape_coef = pi / 6._r8 * props_loc%rho * &
         !    rising_factorial(pgam+1._r8, props_loc%eff_dim)
              gamma(pgam+1._r8+props_loc%eff_dim)/gamma(pgam+1._r8)    ! h1g, 2016-12-09 
     end if
        ...
        if (props_loc%eff_dim == 3._r8) then
           props_loc%shape_coef = pi / 6._r8 * props_loc%rho * &
            !    rising_factorial(pgam(i)+1._r8, 3)
                 gamma(pgam(i)+4._r8)/gamma(pgam(i)+1._r8)  ! h1g 2016-12-09
        else
           props_loc%shape_coef = pi / 6._r8 * props_loc%rho * &
            !    rising_factorial(pgam(i)+1._r8, props_loc%eff_dim)
                 gamma(pgam(i)+1._r8+props_loc%eff_dim)/gamma(pgam(i)+1._r8)  ! h1g 2016-12-09 
        end if
        ! Limit to between 2 and 50 microns mean size.
...
function var_coef_r8(relvar, a) result(res)
...
!  res = rising_factorial(relvar, a) / relvar**a
  res = gamma(relvar+a)/gamma(relvar)/ relvar**a  ! h1g, 2016-12-09

end function var_coef_r8

function var_coef_integer(relvar, a) result(res)
  ! Finds a coefficient for process rates based on the relative variance
  ! of cloud water.
  real(r8), intent(in) :: relvar
  integer, intent(in) :: a
...
subroutine ice_deposition_sublimation(t, qv, qc, qi, ni, &   ! h1g 2020-03-31
  ...
  real(r8), dimension(mgncol), intent(in) :: qc    ! h1g 2020-03-31
  ...
  do i=1,mgncol
     if (qi(i)>=qsmall) then
        ...
        if ( t(i)>tmelt-40.0 .and. t(i)<tmelt .and. qc(i)>qsmall ) then  ! h1g 2020-06-25

           !Compute bergeron rate assuming cloud for whole step.
           berg(i) = max(epsi/ab*(qvl(i) - qvi(i)), 0._r8)
           berg(i) = berg(i) * icldm(i)
        else !T>frz
           berg(i)=0._r8
        end if !T<frz
...
! formula from  Manton and Cotton (1977).  
! This formula has been used in Chen and Cotton (1987), 
! modified for sub-grid distribution of qc
! minimum qc of 1 x 10^-8 prevents floating point error

subroutine cotton_liq_autoconversion(microp_uniform, qcic, &
     ncic, rho, relvar, rthresh, prc, nprc, nprc1, mgncol)

  integer, intent(in) :: mgncol
  logical, intent(in) :: microp_uniform

  real(r8), intent(in) :: rthresh
  ...
  if (.not. microp_uniform) then
    ! prc_coef(:) = var_coef(relvar(:), 2.47_r8)
      do i=1,mgncol
        prc_coef(i) = gamma(relvar(i)+2.33_r8)/gamma(relvar(i))/ relvar(i)**2.33  ! h1g, 2016-12-09
      enddo
  else
     prc_coef(:) = 1._r8
  end if
  ...
  do i=1,mgncol
     rad_liq = 620350.49* (qcic(i)/max(ncic(i),icsmall)/rhow)**(0.33_r8)
     if (qcic(i) >= icsmall .and. rad_liq > rthresh ) then        
        ...
        prc(i) = prc_coef(i) * & 
            32681. * qcic(i)**2.33_r8 * (ncic(i)*rho(i)*rhow)**(-0.33_r8) 
  ...
  enddo
end subroutine cotton_liq_autoconversion
...
subroutine kk2000_liq_autoconversion(microp_uniform, qcic, &
  ...
  if (.not. microp_uniform) then
    ! prc_coef(:) = var_coef(relvar(:), 2.47_r8)
      do i=1,mgncol
        prc_coef(i) = gamma(relvar(i)+2.47_r8)/gamma(relvar(i))/ relvar(i)**2.47  ! h1g, 2016-12-09
      enddo
  else
     prc_coef(:) = 1._r8
  end if
...
subroutine sb2001v2_liq_autoconversion(pgam,qc,nc,qr,rho,relvar,au,nprc,nprc1,mgncol)
  ...
  do i=1,mgncol
     ...
     pra_coef = var_coef(relvar(i), 4._r8)
     ...
     if (qc(i) > qsmall) then
       ...
       au(i) = pra_coef * kc/(20._r8*2.6e-7_r8)* &
...
subroutine sb2001v2_accre_cld_water_rain(qc,nc,qr,rho,relvar,pra,npra,mgncol)
  ...
  do i =1,mgncol
    ...
    pra_coef = var_coef(relvar(i), 1._r8)
...
!     dum(:) = var_coef(relvar, 2)
      do i=1,mgncol
        dum(i) = gamma(relvar(i)+2)/gamma(relvar(i))/ relvar(i)**2   ! h1g, 2016-12-09
      enddo
  else
     dum(:) = 1._r8
  end if
...
subroutine accrete_cloud_water_snow(t, rho, asn, uns, mu, qcic, ncic, qsic, &
     pgam, lamc, lams, n0s, psacws, npsacws, use_const_ELI, ELI, mgncol)   !h1g, 2020-04-16
...
!--> h1g, 2020-04-16
  logical,  intent(in)  :: use_const_ELI
  real(r8),  intent(in) :: ELI
!<-- h1g, 2020-04-16
...
!  real(r8), dimension(mgncol), intent(in) :: accre_enhan
  ...
  if (.not. microp_uniform) then
     do i=1, mgncol
       pra_coef(i) =  gamma(relvar(i)+1.15_r8)/gamma(relvar(i))/ relvar(i)**1.15   ! h1g, 2016-12-09
     enddo
  else
    pra_coef(:) = 1._r8
  end if
...
subroutine evaporate_sublimate_precip(t, rho, dv, mu, sc, q, qvl, qvi, &
     ...
     pre, prds, am_evp_st, evap_RH_cri, subl_RH_cri, mgncol)
  ...
  real(r8)                                :: evap_RH_cri ! critical relative humidity above which rain does not evaporate.
  real(r8)                                :: subl_RH_cri ! critical relative humidity above which snow does not sublimate.
  ...
  ! only calculate if there is some precip fraction > cloud fraction
     ...
     if (precip_frac(i) > dum(i)) then
        ...
        if (qric(i) >= qsmall) then
           ...
           pre(i) = eps*(qclr-evap_RH_cri*qvl(i))/ab
        ...
        if (qsic(i) >= qsmall) then
           ...
           prds(i) = eps*(qclr-subl_RH_cri*qvi(i))/ab
...
subroutine bergeron_process_snow(t, rho, dv, mu, sc, qvl, qvi, asn, &
  ...
  do i=1,mgncol
    ! if (qsic(i) >= qsmall.and. qcic(i) >= qsmall .and. t(i) < tmelt) then  ! h1g 2020-04-01
     if (qsic(i)>= qsmall .and. t(i)<tmelt .and. t(i)>tmelt-40.0 ) then   ! h1g 2020-06-25
        ab = calc_ab(t(i), qvi(i), xxls)
        eps = 2._r8*pi*n0s(i)*rho(i)*Dv(i)* &
             (f1s/(lams(i)*lams(i))+ &
             f2s*(asn(i)*rho(i)/mu(i))**0.5_r8* &
