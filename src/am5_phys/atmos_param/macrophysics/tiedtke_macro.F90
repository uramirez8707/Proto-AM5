...
! U00          threshold rel hum for cloud formation by large-scale condensation.
! u00_profile  the low-level u00 ECMWF profile should be applied?
! eros_scale   normal erosion rate constant for cloud destruction (default = 1.E-06)
! eros_choice  enhanced erosion should occur in turbulent or convective conditions ?
! eros_scale_c erosion rate constant for cloud destruction for convective conditions.
! eros_scale_t erosion rate constant for cloud destruction for turbulent conditions.
! mc_thresh    convective mass-flux threshold for enhanced erosion to turn on.
! diff_thresh  diffusion coefficient threshold for enhanced erosion to turn on.
! efact        factor used in formula to enhance cloud erosion as a function of height

real    :: U00 =0.80
logical :: u00_profile = .true.
real    :: eros_scale     =  1.E-06
logical :: eros_choice    =  .true.
real    :: eros_scale_c   =  8.E-06
real    :: eros_scale_t   =  5.E-05
real    :: mc_thresh      =  0.001   
real    :: diff_thresh   =  0.1      
real    :: efact = 0.
real    :: mizfact = 1.    ! factor for turb/rad cooling/heating contribution to dqs_ls
                           ! (dqs_ls: change in qs due to large scale processes)
real    :: omgfact = 1.    ! factor for adiabatic ascent/descent contribution to dqs_ls
real    :: cmffact = 1.    ! factor for parameterized convection contribution to dqs_ls
real    :: qturbfact = 0.  ! ZNT: 06/08/2021: factor for modifying dqs_ls
                           ! by subtracting dq_turb (orig = 0.: no modif)
...
namelist / tiedtke_macro_nml /  &
                       ...
                       mizfact, omgfact, cmffact,                    & !miz
                       qturbfact   ! ZNT: 06/08/2021
...
!-----------------------------------------------------------------------
!    the next step is the apportionment of the non-convective condensation
!    between liquid and ice phases. Following the suggestion of Rostayn
...
!------------------------------------------------------------------------
      do k=1,kdim
        do j=1,jdim
          do i=1,idim
            dqs_ls(i,j,k) = ((                    &
            (omgfact*Input_mp%omega(i,j,k) + cmffact*grav*C2ls_mp%mc_full(i,j,k))/  &
                                Atmos_state%airdens(i,j,k)/cp_air)  +   &
                           mizfact*Input_mp%radturbten(i,j,k))* &      
                                         dtcloud*Atmos_state%dqsdT(i,j,k) &
                        -qturbfact*Input_mp%qturbten(i,j,k)*dtcloud     ! ZNT: 06/08/2021: change dqs to d(qs-q) [IMCOMPLETE!]
...
!       the case that da_ls is not equal to zero.
!------------------------------------------------------------------------
            dum(i,j,k) = (((omgfact*Input_mp%omega(i,j,k) + cmffact*grav*  &
                             C2ls_mp%mc_full(i,j,k))/ &
                                 Atmos_state%airdens(i,j,k)/cp_air) + &
                            mizfact*Input_mp%radturbten(i,j,k))*dtcloud  ! ZNT: dum is dT
