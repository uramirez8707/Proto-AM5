...
!-----------------------------------------------------------------------
!------- namelist ---------
logical :: using_restart_file = .false. ! if set to .false, restart file
                                        ! will NOT be written by this 
                                        ! module; this will not affect
                                        ! answers as long as job is 
                                        ! restarted on a radiation
                                        ! timestep
logical ::  do_clear_sky_pass= .true. !  are the clear-sky radiation
...
character(len=16) ::   &
                rad_package='sea_esf' !  string defining the radiation
                                      ...
                                      !  values : 'sea_esf'
logical ::     &
         renormalize_sw_fluxes=.true. !  should sw fluxes and the zenith
...
integer ::  rad_time_step = 10800     !  radiative time step in seconds


integer ::  sw_rad_time_step = 3600     !  radiative time step in seconds
logical :: use_single_lw_sw_ts = .false.! lw and sw are integrated
...
!  rad_time_step      The radiative time step in seconds.
!  do_clear_sky_pass  are the clear-sky radiation
!                     diagnostics to be calculated ?
!  zenith_spec  string defining how zenith 
!               angle is computed. acceptable
!               values: 'daily_mean', 'annual_
!               mean', 'diurnally_varying'
!  rad_package  string defining the radiation
!               package being used. acceptable
!               values : 'sea_esf'
!  renormalize_sw_fluxes   should sw fluxes and the zenith
!                          angle be renormalized on each 
!                          timestep because of the 
!                          movement of earth wrt the sun ?
!  rad_date   fixed date for which radiation
!             is to be valid (applies to
!             solar info, ozone, clouds)
!             [yr, mo, day, hr, min, sec]
!  rsd   (repeat same day) - call 
!        radiation for the specified 
!        rad_date (yr,mo,day), but run 
!        through the diurnal cycle (hr,
!        min,sec)
!  use_mixing_ratio  assumes q is mixing ratio
!                    rather than specific humidity
!  doing_data_override   input fields to the radiation
!                        package are being overriden
!                        using data_override_mod ?
!  overriding_temps  temperature and ts fields are
!                    overriden ?
!  overriding_sphum  specific humidity field is
!                    overriden ?
!  overriding_clouds cloud specification fields are
!                    overriden ?
!  overriding_albedo surface albedo field is
!                    overriden ?
!  overriding_aerosol   aerosol fields are overriden ?
!  use_co2_tracer_field use co2 value from co2 tracer field?
!  use_ch4_tracer_field use ch4 value from ch4 tracer field?
!  always_calculate     calculate radiative fluxes and heating 
!                       rates on every call to radiation_driver ?
!  always_calculate     fluxes and heating rates should
!                       be calculatd on each call to
!                       radiation_driver ? (true for
!                        standalone applications)
!  use_uniform_solar_input   the (lat,lon) values used to
!                            calculate zenith angle are
!                            uniform across the grid ?
!  lat_for_solar_input  latitude to be used when uni-
!                       form solar input is activated
!  lon_for_solar_input  longitude to be used when uni-
!                       form solar input is activated
