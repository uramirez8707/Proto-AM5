...
use radiation_driver_mod, only: radiation_diag_type, &
...
!    (0 - 3000 cm-1).
!----------------------------------------------------------------------
!BW real,    dimension(:), allocatable  :: bandlo, bandhi
...
!    calculation.
! --------------------------------------------------------------------
!BW real,    dimension(:), allocatable  :: bdlocm, bdhicm

!---------------------------------------------------------------------
!    iband is frequency band index to be used in combined band 
!    calculations. 
!---------------------------------------------------------------------
!BW integer, dimension(:), allocatable  :: iband
...
!#####################################################################
subroutine radiation_diag_init (lonb, latb, npts)
...
!----------------------------------------------------------------------
!BW     nbly              = size(Lw_tables%bdlocm(:))
!BW     nblw              = size(Lw_tables%bandlo(:))
!BW     n_continuum_bands = size(Lw_tables%iband(:))

!BW     allocate ( bdlocm (nbly) )
!BW     allocate ( bdhicm (nbly) )
!BW     allocate ( iband  (n_continuum_bands) )
!BW     allocate ( bandlo (nblw) )
!BW     allocate ( bandhi (nblw) )

!BW     bdlocm(:) = Lw_tables%bdlocm(:)
!BW     bdhicm(:) = Lw_tables%bdhicm(:)
!BW     iband (:) = Lw_tables%iband(:)
!BW     bandlo(:) = Lw_tables%bandlo(:)
!BW     bandhi(:) = Lw_tables%bandhi(:)
...
!####################################################################
subroutine radiation_diag_driver (is, ie, js, je, Rad_diag)
...
type(radiation_diag_type), intent(in) :: Rad_diag
...
!      Rad_diag
!                   radiation_diag_type variable containing diagnostic
!                   longwave and shortwave output used by the
...
!---------------------------------------------------------------------
      do j=js,je
        if (do_raddg(j)) then
          call radiag (is, ie, js, je, j, Rad_diag)
        endif
      end do
...
!#####################################################################
subroutine radiag (is, ie, js, je, jrow, Rad_diag)
...
!      Rad_diag
!                   radiation_diag_type variable containing
!                   diagnostic radiation output used by the radiation
!                   diagnostics module
!
!---------------------------------------------------------------------
...
!
!---------------------------------------------------------------------
      real, dimension (size(Rad_diag%press,3) - 1 ) ::   &
...
!
!----------------------------------------------------------------------
      real, dimension (size(Rad_diag%press,3)   ) ::   &
...
!---------------------------------------------------------------------
      real, dimension (size(Rad_diag%press,3),        &
                       size(Rad_diag%Lw_diagnostics%flx1e1f,3) ) :: flx7, flx7cf
...
!---------------------------------------------------------------------
      real, dimension (size(Rad_diag%press,3)-1,    &
                       size(Rad_diag%Lw_diagnostics%flx1e1f,3))  :: htem7 
...
!---------------------------------------------------------------------
      real, dimension (size(Rad_diag%press,3)-1, &
                       size(Rad_diag%Lw_tables%bdlocm(:))) :: exctsn
...
!                [ Watts / m**2 , or kg / sec**3 ]
!---------------------------------------------------------------------
      real, dimension (size(Rad_diag%Lw_tables%bdlocm(:))) :: &
                                            ftopac, ftopn, vsumac
...
!---------------------------------------------------------------------
      ...
      real    :: ftopeft   ! flux at toa summed over nbtrge bands in 
                           ...
                           ! [ Watts / m**2 , or kg / sec**3 ]
      real    :: cldextdz  ! exp (-cldext) = cloud sw extinction     
                           ! [ dimensionless ]
...
!---------------------------------------------------------------------
     ...
     !integer :: nz
      ...
      if (Rad_diag%do_lwaerosol_forcing) then
        Lw_output_ad = Rad_diag%Lw_output(Rad_diag%indx_lwaf)
      endif

!---------------------------------------------------------------------
!    number of longwave frequency bands
!---------------------------------------------------------------------
     nbly = size(Rad_diag%Lw_tables%bdlocm(:))
     n_continuum_bands = size(Rad_diag%Lw_tables%iband(:))

!---------------------------------------------------------------------
!    define the vertical grid dimension and the number of h2o bands in 
!    the 1200 - 1400 cm-1 range.
!---------------------------------------------------------------------
      ke     = size (Rad_diag%press,3) - 1
      nbtrge = size (Rad_diag%Lw_diagnostics%flx1e1f,3)
...
!     nential-sum-fit parameterization
!---------------------------------------------------------------------
...
!    out along with the cloud amount. 
!    use the contents of Atmos_input for deltaz
...
!---------------------------------------------------------------------
               ...
               !DEL            Atmos_input%clouddeltaz(iloc,jloc,k)* &
               !DEL            1.0E-03
                      if (cldextdz > 0.0) then           
                        cldssalb = Rad_diag%cldsct(iloc,jloc,k,n,1)/ &
                                   Rad_diag%cldext(iloc,jloc,k,n,1)
                      else
                        cldssalb = 0.0
                      endif
...
!----------------------------------------------------------------------
!BW           else
!BW             call error_mesg ('radiation_diag_mod', &
!BW                    'no shortwave clouds are activated', FATAL)
!BW           endif
...
!--------------------------------------------------------------------
            cvisrfgd_dir = Rad_diag%asfc_vis_dir(iloc,jloc)
...
!----------------------------------------------------------------------
!  write out the astronomical data used for this shortwave calculation
...
!----------------------------------------------------------------------
            do k=ks,ke+1
              pflux(k)  = Rad_diag%pflux(iloc,jloc,k)
              press(k)  = Rad_diag%press(iloc,jloc,k)
            end do
            ...
            write(radiag_unit,9110) (k, press (k),   &
                                     ...
                                     Rad_diag%Lw_output(1)%flxnet(iloc,jloc,k), &
                                     pflux             (k), k=ks,ke)
...
!----------------------------------------------------------------------
            do k=ks,ke+1
              flx1(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,1)
              flx2(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,2)
              flx3(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,3)
              flx4(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,4)
              flx5(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,5)
              flx6(k) = Rad_diag%Lw_diagnostics%fluxn(iloc,jloc,k,6)
              if (Rad_diag%do_totcld_forcing) then
                flx1cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,1)
                flx2cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,2)
                flx3cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,3)
                flx4cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,4)
                flx5cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,5)
                flx6cf(k) = Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,6)
              endif
              if (nbtrge > 0) then
                do m=1,nbtrge
                  if (Rad_diag%do_totcld_forcing) then
                    flx7cf(k,m) =     &
                             Rad_diag%Lw_diagnostics%fluxncf(iloc, jloc, k,6+m)
                  endif
                  flx7(k,m) = Rad_diag%Lw_diagnostics%fluxn(iloc, jloc, k,6+m)
                end do
              endif
            end do
...
!----------------------------------------------------------------------
            do k=ks,ke
              ctsco2(k) = Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,2)
              ctso3(k)  = Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,5)
              cts (k)   = Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,1)
              cts (k)   = cts(k) +      &
                          Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,3)
              cts (k)   = cts(k) +     &
                          Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,4)
              cts (k)   = cts(k) +    &
                          Rad_diag%Lw_diagnostics%cts_out(iloc,jloc,k,6)
            end do
            do k=ks,ke
...
!--------------------------------------------------------------------
            do n=1,nbly-1
              ...
              ftopn(n) = Rad_diag%Lw_diagnostics%fctsg(iloc,jloc,n) - qsum
            end do
            ftopn(nbly) = 0.0E+00
...
!---------------------------------------------------------------------- 
            ftopac(1) = ftopn(1)
            vsumac(1) = Rad_diag%Lw_diagnostics%fctsg(iloc,jloc,1)
            do n=2,nbly
              ftopac(n) = ftopac(n-1) + ftopn(n)
              vsumac(n) = vsumac(n-1) +           &
                            Rad_diag%Lw_diagnostics%fctsg(iloc,jloc,n)
            end do
