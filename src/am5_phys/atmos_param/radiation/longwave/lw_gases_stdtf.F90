...
namelist /lw_gases_stdtf_nml/  do_coeintdiag,    &
                               NSTDCO2LVLS,      &
                               llco2, llch4, lln2o
!---------------------------------------------------------------------
!------- public data ------
...
!  intent(in) variables:
!
!     index   the spectral index CTS transmission function is computed 
!     Optical CFC gas optical depth
!     cfc_tf  CTS transmission function output
!
...
!  intent(in) variables:
!
!     index   the spectral index where exact CTS transmission function is computed
!     klevel  the level below which exact CTS transmission function is computed
!     Optical the CFC gas optical depth
!
!  intent(out) variables:
!
!     cfc_tf  the exact CTS transmission function output
!
!----------------------------------------------------------------------
...
!
!     index   the spectral index where exact CTS transmission function is computed
!     Optical the CFS gas optical depth
!
!  intent(out) variables:
!
...
!     nklo, nkhi    vertical level pairs:  upper and lower level index
!     nkplo, nkphi  pressure level pairs:  upper and lower level index
!     ca_app        the interpolation coefficient
!
!---------------------------------------------------------------------
...
! ndimkp, ndimk  extents of dimensions for output 
!                interpolation transmissivity arrays.
!  dgasdt10_lvl gasp10_lvl d2gast10_lvl dgasdt8_lvl 
!  gasp8_lvl d2gast8_lvl
!       variables used in do_lvlcalc calculation path

!  dgasdt10_lvlcts gasp10_lvlcts d2gast10_lvlcts 
!  dgasdt8_lvlcts gasp8_lvlcts d2gast8_lvlcts
!        variables used in do_lvlctscalc calculation path

!  dgasdt10_lyr gasp10_lyr d2gast10_lyr
!  dgasdt8_lyr gasp8_lyr d2gast8_lyr
...
!      ca, xa, sexp, uexp
!          coefficients in the transmission function between two pressure levels
!
!-------------------------------------------------------------------
...
!       cav, xav, sexpv, uexpv   coefficients in the 
!                                transmission function between two pressure levels  
!
!--------------------------------------------------------------------
...
!  intent(in) variables:
!
!    press_hiv, pres_lov  high and low pressure pair 
...
!    sexpintv, caintv, xaintv, uexpintv  coefficients in the transmission
!    function between two pressure levels
!
!-------------------------------------------------------------------
...
!---------------------------------------------------------------------
! subroutine to inverse coefficients from transmission functions using
! newton method (2 dimensional)
!---------------------------------------------------------------------
