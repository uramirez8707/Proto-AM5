...
character(len=32)  :: filename = 'o3.climatology.nc'
...
character(len=24)  ::   gfdl_zonal_ozone_type = 'seasonally_varying'
                      ...
                      ! 'seasonally_varying'
!BW logical            ::   do_mcm_o3_clim = .false.
!BW                       ! treat ozone as in the manabe climate model ?
integer, dimension(6) ::       &
...
!######################################################################
subroutine ozone_driver (is, ie, js, je, lat, Rad_time, pflux, &
                         r, Rad_gases )
...
!                   model levels where pflux(nlev+1) is the
!                   surface pressure
!
...
!
!---------------------------------------------------------------------
!BW   real, dimension (size(Atmos_input%press,1), &
!BW                    size(Atmos_input%press,2), &
!BW                    size(Atmos_input%press,3))    :: phaf
...
!BW     do k=1,kmax+1
!BW       if (do_mcm_o3_clim) then
!BW         phaf(:,:,k) = 100000.*Atmos_input%phalf(:,:,k)/  &
!BW                       Atmos_input%phalf(:,:,kmax+1)
!BW       else
     !BW    phaf(:,:,k) = pflux(:,:,k)
...
!---------------------------------------------------------------------
        if (do_gfdl_zonal_ozone) then
          call gfdl_zonal_ozone (Rad_time, lat, pflux, Rad_gases%qo3)
...
!---------------------------------------------------------------------
          ...
          call get_clim_ozone (is, js, Rad_time, pflux, Rad_gases%qo3)
...
!BW           ozone(i,j,k) = 0.0
!BW           do l=1,81
!BW             if ((ph(l+1) >= phalf(i,j,k)) .and.   &
!BW                 (ph(l) <= phalf(i,j,k+1))) then
!BW               if ((ph(l+1) < phalf(i,j,k+1)) .and.    &
!BW                   (ph(l) < phalf(i,j,k)))   then
!BW                 ozone(i,j,k) = ozone(i,j,k) + profile(l)*   &
!BW                                (ph(l+1) - phalf(i,j,k))
!BW               endif
!BW               if ((ph(l+1) < phalf(i,j,k+1)) .and.    &
!BW                   (ph(l) >= phalf(i,j,k)))   then  
!BW                 ozone(i,j,k) = ozone(i,j,k) + profile(l)*    &
...
!BW         endif ! do_mcm_o3_clim if block
...
!---------------------------------------------------------------------
!BW         if (.not.do_mcm_o3_clim .and. ph(82) < phalf(i,j,k+1)) then
            if (ph(82) < phalf(i,j,k+1)) then
...
!----------------------------------------------------------------------
!BW         if (do_mcm_o3_clim) then
!BW           if (ozone(i,j,k) <= 0.0) then
!BW             do l=1,81
!BW               if (ph(l) < phalf(i,j,k) .and.     &
!BW                   ph(l+1) > phalf(i,j,k+1) ) then
!BW                 ozone(i,j,k) = profile(l)
!BW               endif
!BW             end do
!BW           endif
!BW         endif 
