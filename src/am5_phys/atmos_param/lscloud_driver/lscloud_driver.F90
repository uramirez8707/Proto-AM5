...
!-----------------------------------------------------------------------
!  Dmin  minimum permissible dissipation in analytic integration of qa, ql, qi
...
!  cfact  factor in bergeron process calculation when the effect of dust
!         is not considered; values > 1 enhance the conversion to ice.
!  microphys_scheme  the microphysics scheme being used (currently either
!                    "rotstayn_klein" or "mg2")
!  macrophys_scheme  the macrophysics scheme being used (currently "tiedtke")
!  aerosol_activation_scheme  the aerosol activation scheme being used
!                             (currently either "dqa" or "total")
!  do_dust_berg  sub-micron dust particles are used as ice nuclei for bergeron
!                process evaluation?
!  super_ice_opt    flag to indicate how to treat supersaturation; 0 => don't allow.
!  do_ice_nucl_wpdf should we use activate ice nuclei by assuming a pdf ?
!  do_pdf_clouds    the statistical cloud scheme should be run?
!  betaP            p-parameter to the beta distribution - used when do_pdf_clouds is true
!  qthalfwidth  half-width to the qt PDF - used when do_pdf_clouds is true and
!               diagnostic variance
...
!  nsublevels  number of sublevels to vertical sub-grid cloud structure - used when
!              do_pdf_cloud is true
!  kmap  PPM partial remap integer - used when do_pdf_cloud is true and if
!        vertical subgrid structure is used
!  kord  PPM method number - used when do_pdf_cloud is true and if vertical
!        subgrid structure is used
!  pdf_org  define pdf based on input water fields (rather than updated ones) ?
!-----------------------------------------------------------------------
...
character(len=64)  :: aerosol_activation_scheme = 'total'
