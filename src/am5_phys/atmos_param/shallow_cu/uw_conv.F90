...
!-------  interfaces --------
  ...
  logical :: do_rescale   = .true.
  logical :: do_rescale_t = .true.
  ...
  integer :: tracer_check_type = 2 !legacy
  ...
  NAMELIST / uw_conv_nml / iclosure, rkm_sh1, rkm_sh, cldhgt_max, plev_cin, plev_omg, eis_max,eis_min,do_peff_land, &
       ...
       use_pblhttke_avg, use_hlqtsrc_avg, use_capecin_avg, do_plev_umf, plev_umf, shallow_umf_thresh, &
  ...
  real    :: rpen     = 4.0    ! for entrainment efficiency
  real    :: rmaxfrac = 0.1    ! maximum allowable updraft fraction
  real    :: wmin     = 0.5    ! minimum vertical velocity for computing updraft fraction
  real    :: wmax     = 20.    ! maximum allowable vertical velocity
  ...
  real    :: auto_th0 = 0.2e-3 ! threshold for precipitation
  real    :: auto_rate= 1.e-3
  real    :: tcrit    = -25.15  ! critical temperature
  real    :: deltaqc0 = 0.5e-3
  logical :: do_pdfpcp= .false.
  logical :: do_pmadjt= .true.
  logical :: do_emmax = .true.
  logical :: do_pnqv  = .true.
  logical :: do_tten_max = .false.
  logical :: include_emf_s = .false.
  real    :: rad_crit = 14.0   ! critical droplet radius
  real    :: emfrac_max = 0.25
  integer :: mixing_assumption = 6
  integer :: mp_choice = 3
  integer :: de_choice = 0
  real    :: Nl_land   = 300.e6
  ...
  real    :: r_thresh  = 10.
  logical :: do_pevap = .true.
  real    :: cfrac     = 0.15
  real    :: hcevap    = 0.85
  real    :: hcevappbl = 0.95
  real    :: pblfac    = 1.0
  logical :: do_new_pblfac = .true.
  logical :: do_weffect = .false.
  logical :: do_limit_wmax =.true.
  logical :: do_limit_fdr  =.true.
  real    :: weffect    = 0.5
  real    :: peff_l     = 6.0e-5
  real    :: peff_i     = 11.e-5
  real    :: scaleh0    = 1000.
  ...
  real    :: rkfre    = 0.1    ! vertical velocity variance as fraction of tke
  real    :: tau_sh   = 7200.  !
  real    :: wcrit_min= 0.2
  real    :: mass_fact= 0.25
...
!========Option for deep convection=======================================
  ...
  real    :: rkm_dp1       = 0.9
  real    :: rkm_dp2       = 0.1
  real    :: crh_th_ocean  = 0.4
  real    :: crh_th_land   = 0.4
  real    :: crh_max       = 1.0
  real    :: cape_th       = 10.
  real    :: cin_th        = 5.
  real    :: cwfn_th       = 10.0
  ...
  real    :: hcevap_d    = 0.85
  real    :: pblfac_d    = 1.0
  real    :: hcevappbl_d = 0.95
  real    :: dcapedm_th  = 0
  real    :: dcwfndm_th  = 100
  real    :: frac_limit_d = 0.25
  real    :: lofactor_d   = 1.0
  real    :: auto_th0_d   = 0.2e-3
  real    :: tcrit_d      = -25.15
  real    :: peff_l_d     = 6.0e-5
  real    :: peff_i_d     = 11.e-5
  ...
  logical :: do_cgust_dp  = .true.
  logical :: include_emf_d = .false.
...
!#####################################################################
  ...
  SUBROUTINE uw_conv(is, js, Time, tb, qv, ub, vb, pmid,pint,zmid,zint, & !input
       ...
       tten_gw, cqa_gw) !p1l: output for Beres gravity wave scheme
...
!required when do_mse_budget=T
    ...
    real, intent(inout), dimension(:,:,:) :: tten_gw  ! grid-mean convective temperature tendency to be used in gw_beres
    real, intent(inout), dimension(:,:,:) :: cqa_gw  ! updraf area to be used in gw_beres
...
!=========Prepare variables to be passed to and used in gw_beres==================
! grid-mean convective heating rate from the deep plume
    tten_gw = tten_d
! updraft area from the deep plume interpolated from layer interfaces to layer center
    do i = 1, imax
      do j = 1, jmax
        do k = 1, kmax
          cqa_gw(i,j,k)=(cqa_d(I,j,k)+cqa_d(i,j,k+1))*0.5
        enddo
      enddo
    enddo
