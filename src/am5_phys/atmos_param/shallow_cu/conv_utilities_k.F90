...
!++++yim     
 ...
 type adicloud
    ...
    real, _ALLOCATABLE :: qi (:)_NULL, thc (:)_NULL, qct (:)_NULL, qs (:)_NULL
    real, _ALLOCATABLE :: thv(:)_NULL, nu  (:)_NULL, leff(:)_NULL, dqsdp_adi(:)_NULL
...
!#####################################################################
  ...
  subroutine ac_init_k(kd, ac)
    ...
    allocate ( ac%qs    (1:kd)); ac%qs   =0.;
    allocate ( ac%dqsdp_adi (1:kd)); ac%dqsdp_adi =0.;
...
!#####################################################################
  ...
  subroutine ac_clear_k(ac)
    ...
    ac%qi   =0.;    ac%thc  =0.;    ac%qct  =0.; ac%qs   =0.;
    ac%thv  =0.;    ac%nu   =0.;    ac%leff =0.; ac%hl   =0.;
    ac%buo  =0.;    ac%buog =0.;    ac%dbuodp=0.;ac%dqsdp_adi=0.;
...
!#####################################################################
  ...
  subroutine ac_end_k(ac)
    type(adicloud), intent(inout) :: ac
    deallocate (ac%t, ac%qv, ac%ql, ac%qi, ac%thc, ac%qct, ac%qs, ac%dqsdp_adi, &
...
!#####################################################################
  ...
  do k=1,kl-1
     ...
     ac%dqsdp_adi(k)=(ac%qs(k+1)-ac%qs(k)) /sd%dp(k)
  end do
