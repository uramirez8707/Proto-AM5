...
use        hanedmf_mod, only: hanedmf_init, hanedmf    ! ZNT 04/27/2020
...
!---------------- private data -------------------
 ...
 integer :: nqn, nqni        !  ZNT 03/20/2023: tracer indices for liq_drp and ice_num
 integer :: ntke             !  ZNT 11/21/2022: restored -- tracer index for TKE
...
!-------------------- namelist -----------------------------------------
 ...
 logical :: do_stable_bl  = .true.      ! Do the local Ri-based ED scheme
                                        !  * For stable PBL and free troposphere
 logical :: do_entrain    = .true.      ! Do the Lock k-profile ED scheme
                                        !  * For convective PBL only

 ! Do the NCEP TKE-based EDMF scheme 
 ! The previous diag/mfonly/full flags are merged as a single 'edmf_level' flag in hanedmf.F90
 logical :: do_hanedmf    = .false.     ! Do the NCEP EDMF scheme

 character(len=24) :: gust_scheme  = 'beljaars' ! valid schemes are:
                                                !   => 'constant'
                                                !   => 'beljaars'
 ...
 ! ZNT, 10/13/2020
 real :: kt_haned_min = 0.1 ! Below this Kt threshold the TKE-EDMF is deemed inactive 
                            ! and the stable pbl scheme can kick in
                            ! ** This threshold is NOT used when stable pbl scheme is disabled.
 ...
 namelist /vert_turb_driver_nml/ gust_scheme, constant_gust, &
                                 ...
                                 do_hanedmf, &       ! ZNT 06/05/2020
                                 kt_haned_min        ! ZNT 10/13/2020
...
type(time_type), intent(in)         :: Time, Time_next
   ...
   real, intent(in), dimension(:,:) :: u_ref, v_ref, tau_x, tau_y, t_surf_rad,   &  ! ZNT
                                       shflx, lhflx,                             &  ! ZNT
                                       wind, thv_atm, thv_surf                      ! ZNT
logical, intent(in), dimension(:,:) :: convect       
   real, intent(in), dimension(:,:,:) :: tdtlw, p_half, p_full, &
                                         z_half, z_full, &
                                         u, v, t, q, um, vm, tm, qm  ! ZNT 06/05/2020
   real, intent(inout), dimension(:,:,:) :: udt, vdt, tdt, qdt  ! ZNT 06/05/2020: changed to inout
   real, intent(inout), dimension(:,:,:,:) :: rdt               ! ZNT 10/11/2020: changed to inout
...
integer, intent(in),optional, dimension(:,:) :: kbot
   ...
   type(aerosol_type), intent(in),optional :: Aerosol     ! ZNT 06/23/2023
...
real   , dimension(size(t,1),size(t,2),size(t,3))   :: qn_in, qni_in    ! ZNT: 2023-05-03
...
real   , dimension(size(t,1),size(t,2),size(t,3))   :: &      
         udt_hanmf, vdt_hanmf, tdt_hanmf, qdt_hanmf,   & ! ZNT 06/05/2020
         qldt_hanmf, qidt_hanmf, qadt_hanmf,           & ! ZNT 10/11/2020
         qndt_hanmf, qnidt_hanmf                         ! ZNT 2023-05-03: adding qn and qni
real   , dimension(size(diff_t,1),size(diff_t,2),size(diff_t,3)) :: &
         diff_t_haned, diff_m_haned
real   , dimension(size(t,1),size(t,2))   :: zpbl_han    ! ZNT 12/08/2020
logical :: use_z_above_sfc     ! ZNT 03/08/2023
...
!---- set up state variable used by this module ----
      ...
      !-- ZNT: 2023-05-03: zero-out the cloud tracer arrays --------------
      !-- qlin, qiin, qain originally initialized in the if-block below --
      qlin = 0.0   
      qiin = 0.0
      qain = 0.0
      qn_in = 0.0 
      ...
      if (doing_prog_clouds) then
           nt=size(r,4)
           if (nt == 0 .or. nt < max(nql,nqi,nqa,nqn,nqni))   &                          ! ZNT: 2023-05-03
        call error_mesg ('vert_turb_driver',                  & 
                     'number of tracers less than nql or nqi or nqa or nqn or nqni', &   ! ZNT: 2023-05-03
      FATAL) 
           if (use_tau) then
                qlin (:,:,:)=r(:,:,:,nql)
                qiin (:,:,:)=r(:,:,:,nqi)
                qain (:,:,:)=r(:,:,:,nqa)
                if (nqn > 0)  qn_in (:,:,:)=r(:,:,:,nqn)                                 ! ZNT: 2023-05-03
                if (nqni > 0) qni_in(:,:,:)=r(:,:,:,nqni)                                ! ZNT: 2023-05-03
           else
                qlin (:,:,:)=rm(:,:,:,nql)+rdt(:,:,:,nql)*dt
                qiin (:,:,:)=rm(:,:,:,nqi)+rdt(:,:,:,nqi)*dt
                qain (:,:,:)=rm(:,:,:,nqa)+rdt(:,:,:,nqa)*dt
                if (nqn > 0)  qn_in (:,:,:)=r(:,:,:,nqn) +rdt(:,:,:,nqn) *dt             ! ZNT: 2023-05-03
                if (nqni > 0) qni_in(:,:,:)=r(:,:,:,nqni)+rdt(:,:,:,nqni)*dt             ! ZNT: 2023-05-03
           endif
      end if
...
! --- Han's EDMF
   if (do_hanedmf) then
     if (present(Aerosol)) then
       call hanedmf (is,ie,js,je,dt,Time_next,tdtlw,rough,u_star,b_star,  &
                     u_ref,v_ref,tau_x,tau_y,t_surf_rad,shflx,lhflx,      & ! ZNT - t_surf_rad required
                     wind, thv_atm, thv_surf,                             & ! ZNT 
                     tt,qq,qlin,qiin,qain,qn_in,qni_in, &                   ! ZNT 2023-05-03: adding qn and qni
                     uu,vv,z_full,p_full,z_half,p_half, &
                     rdiag(:,:,:,ntke), &                                   ! ZNT - TKE as a diag tracer
                     udt_hanmf, vdt_hanmf, tdt_hanmf, qdt_hanmf, &  ! ZNT 06/05/2020
                     qldt_hanmf, qidt_hanmf, qadt_hanmf,         &  ! ZNT 10/11/2020
                     qndt_hanmf, qnidt_hanmf,                    &  ! ZNT 2023-05-03: adding qn and qni
                     diff_m_haned, diff_t_haned,                 &  ! ZNT 06/05/2020
                     zpbl_han, use_z_above_sfc,                  &  ! ZNT 12/08/2020, 03/08/2023
                     asol=Aerosol)                                  ! ZNT 06/27/2023
     else  ! ZNT 06/27/2023: no aerosol input
       call hanedmf (is,ie,js,je,dt,Time_next,tdtlw,rough,u_star,b_star,  &
                     u_ref,v_ref,tau_x,tau_y,t_surf_rad,shflx,lhflx,      & ! ZNT - t_surf_rad required
                     wind, thv_atm, thv_surf,                             & ! ZNT 
                     tt,qq,qlin,qiin,qain,qn_in,qni_in, &                   ! ZNT 2023-05-03: adding qn and qni
     ...
     tdt = tdt + tdt_hanmf
     qdt = qdt + qdt_hanmf
     rdt(:,:,:,nql) = rdt(:,:,:,nql) + qldt_hanmf  ! ZNT 10/11/2020
     rdt(:,:,:,nqi) = rdt(:,:,:,nqi) + qidt_hanmf  ! ZNT 10/11/2020
     rdt(:,:,:,nqa) = rdt(:,:,:,nqa) + qadt_hanmf  ! ZNT 10/11/2020

     ! ZNT: 2023-05-03: adding MF tendencies for qn and qni
     if (nqn > 0)  rdt(:,:,:,nqn)  = rdt(:,:,:,nqn)  + qndt_hanmf 
     if (nqni > 0) rdt(:,:,:,nqni) = rdt(:,:,:,nqni) + qnidt_hanmf

     ! ED coefficients and PBL height
     ! ZNT: 2024-01-25: changed from sum to max; 
     !      should not affect results when do_entrain = .false., 
     !      or when do_entrain = .true. and edmf_level < 2.
     diff_m = MAX(diff_m, diff_m_haned)
     diff_t = MAX(diff_t, diff_t_haned)
     if (use_z_above_sfc) then   ! ZNT 03/08/2023
         z_pbl = MAX(z_pbl, zpbl_han)
     else
         z_pbl = MAX(z_pbl, zpbl_han - z_half(:,:,nlev+1))
     endif
   endif
...
! <-- h1g, 2012-08-08
   ...
   if( do_stable_bl ) then
        ...
        if (do_entrain) then
            ...
            ! --->h1g, 2012-07-16; ZNT 10/13/2020 
            if (do_hanedmf) then
            ! ZNT 10/13/2020: Add option to disable stable scheme in PBL, mimics CLUBB logic
                where (diff_t_haned <= kt_haned_min)    ! ZNT: stable scheme is permitted 
                                                        ! when Han ED is lower than the threshold
                                                        ! 01/25/2024: always true when edmf_level < 2
...
!--->h1g: calculate relative humidity, 2015-04-02
         if ( id_rh_Ri_025 > 0 ) then
           call rh_calc ( p_full,  tt, qq, RH_3D_tmp, do_cmip=.true.)
...
! ZNT 03/21/2023: get tracer indices for liq_drp and ice_num
          nqn  = get_tracer_index ( MODEL_ATMOS, 'liq_drp' )
          nqni = get_tracer_index ( MODEL_ATMOS, 'ice_num' )
       if (mpp_pe() == mpp_root_pe()) then
          if (nqn > 0) then
              write (logunit,'(a,i4)') 'Cloud droplet number index: nqn =',nqn
          else
              write (logunit,'(a)') 'Cloud droplet number index nqn NOT FOUND'
          endif
          if (nqni > 0) then
              write (logunit,'(a,i4)') 'Cloud ice number index: nqni =',nqni
          else
              write (logunit,'(a)') 'Cloud ice number nqni NOT FOUND'
          endif
       endif
...
!-------------Han's EDMF, ZNT 06/05/2020------------------------------
      if (do_hanedmf) then
         ntke = get_tracer_index ( MODEL_ATMOS, 'tke' )  ! ZNT 04/27/2020 - diagnostic TKE now
         ! tke must be a diagnostic tracer
         if (ntke <= ntp) call error_mesg ('vert_turb_driver_mod', &
                    'tke can not be a prognostic tracer', FATAL)
         call hanedmf_init ( axes, Time )   ! ZNT 04/27/2020
      endif
