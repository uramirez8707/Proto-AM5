...
!--- Physics type definition
 ...
 type physics_type
     ...
     type (physics_input_block_type), dimension(:), allocatable :: block 
...
!--- Physics tendency type definition
 ...
 type physics_tendency_type
     type (physics_tendency_block_type), dimension(:), allocatable :: block 
 ...
 type phys_mp_exch_type
      ...
      real, dimension(:,:,:), pointer :: qturbten => null()       ! ZNT: 06/07/2021
   ...
   integer, dimension(:,:,:), pointer :: exist_dpconv=> null() 
      ...
      real, dimension(:,:,:), pointer :: tten_gw => null()
      real, dimension(:,:,:), pointer :: cqa_gw => null()
...
!---deallocate input block data
   do n = 1, size(Physics%block,1)
     ...
     if (allocated(Physics%block(n)%tmp_4d)) &
